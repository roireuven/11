#!/usr/bin/env python3
"""Add invoice i18n keys to all app locale JSON files in public/assets/locales/."""
...
KEYS_FILE = ROOT / "doc" / "i18n" / "invoice-app-keys.json"
MARKER = "hrmm-invoice-keys-v1"
...
def patch_locale_file(path: Path, all_keys: dict) -> bool:
    ...
    for section in ("invoice", "msg", "common"):
    ...
    if meta.get("invoiceKeys") != MARKER:
        meta["invoiceKeys"] = MARKER
        changed = True
...
def main() -> int:
    ...
    print(f"Locale invoice keys: {count} file(s) updated")
    return 0
