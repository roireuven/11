#!/usr/bin/env python3
"""Apply mobile double-bars i18n keys to locale JSON files."""
...
KEYS_FILE = ROOT / "doc" / "i18n" / "mobile-double-bars-keys.json"
...
def main() -> int:
    if not LOCALES_DIR.is_dir() or not KEYS_FILE.is_file():
        print("Skip mobile double-bars locale keys")
        return 0
    ...
    for path in sorted(LOCALES_DIR.glob("*.json")):
        ...
        if apply_keys(data, flat):
            ...
            count += 1
    print(f"Locale mobile double-bars keys: {count} file(s) updated")
    return 0
