...
- **Full product documentation** for HotelRestaurantMini-MartManagement is in [`doc/`](doc/). Deploy app + docs to Firebase: `npm install && firebase login && npm run deploy` (builds app with Documentation button + `/doc/` site). CI: `.github/workflows/deploy-firebase.yml` needs `FIREBASE_TOKEN` secret.
